        ...
        <div className={cardClass}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            ...
            <div>
              ...
              <input type="text" name="customerPhone2" value={newItem.customerPhone2} onChange={handleInputChange} placeholder="Např. 606 123 456" className={inputClass} value={newItem.customerContact} onChange={handleInputChange} placeholder="Např. 777 123 456" className={inputClass} />
            </div>
          </div>
  ...
  const handlePrint = () => {
    ...
    const formatPhone = (raw) => {
      ...
      if (digits.startsWith("+420")) {
        const d = digits.replace("+420", "").replace(/\D/g, "");
        return "+420 " + d.replace(/(\d{3})(?=\d)/g, "$1 ").trim();
      }
      const just = digits.replace(/\D/g, "");
      return just.replace(/(\d{3})(?=\d)/g, "$1 ").trim();
    };

    // build rows from filteredDeliveries
    const rowsHtml = filteredDeliveries.map((d, idx) => {
      const itemName = escapeHtml(d.itemName || "-");
      const note = escapeHtml(d.note || "-");
      const price = formatCurrencyCZ0(d.sellingPriceCzk);
      const city = escapeHtml(d.deliveryCity || "-");
      const address = escapeHtml(d.customerAddress || "-");
      const tel1 = formatPhone(d.customerContact || "");
      const tel2 = formatPhone(d.customerPhone2 || "");
      const telBlock = tel2 ? `${tel1}<br>${tel2}` : tel1;

      return `<tr>
        <td>${itemName}</td>
        <td>${note}</td>
        <td style="text-align:right">${price} Kč</td>
        <td>${city}</td>
        <td>${address}</td>
        <td style="line-height:1.3">${telBlock}</td>
      </tr>`;
    }).join("");
    ...
    const html = `
      <html>
        <head>
          <title>Seznam rozvozů pro ${escapeHtml(selectedDriver)} - ${escapeHtml(weekNumber)}</title>
          <style>
            @media print {
            ...
            h2 { font-size: 14px; margin: 0 0 14px 0; color: #555; }
            table { width: 100%; border-collapse: collapse; }
            th, td { border: 1px solid #ddd; padding: 8px 10px; vertical-align: top; }
            th { background: #f3f4f6; text-transform: uppercase; font-size: 11px; letter-spacing: .02em; text-align: left; }
            td { font-size: 13px; }
            td:nth-child(3) { text-align: right; } /* Prodej v Kč */
          </style>
        </head>
        <body>
          <h1>Seznam rozvozů</h1>
          <h2>Řidič: ${escapeHtml(selectedDriver)} — ${escapeHtml(weekNumber)}</h2>
          <table>
            <thead>
              <tr>
                <th>Položka</th>
